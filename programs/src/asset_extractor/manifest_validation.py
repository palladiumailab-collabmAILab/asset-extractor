"""Standard-library validation for the core extraction run manifest.

The validator intentionally remains dependency-free for field-level checks and
output hash verification. JSON-Schema validation is an additional CI contract
provided by :mod:`asset_extractor.schema`; keeping the two layers separate lets
the command validate a report on a minimal recovery environment.
"""
...
def _validate_output_tree(manifest: dict[str, Any], root: Path, errors: list[str]) -> None:
    ...
    try:
        ...
        return
    if actual != manifest["outputs"]["files"]:
...
def _validate_manifest_metadata(manifest: dict[str, Any], errors: list[str]) -> None:
    ...
    if _check_keys(
        limits,
        {"max_entries", "max_member_bytes", "max_total_bytes", "max_ratio"},
        "limits",
        errors,
    ):
        ...
        if (
            not isinstance(limits["max_ratio"], (int, float))
            or isinstance(limits["max_ratio"], bool)
            or limits["max_ratio"] <= 0
        ):
    ...
    if _check_keys(normalized, {"profile", "strict", "limits"}, "normalized_config", errors):
        if (
            normalized["profile"] != manifest["profile"]
            or normalized["strict"] != manifest["strict"]
            or normalized["limits"] != limits
        ):
...
def _validate_scan_entry(item: Any, label: str, errors: list[str]) -> None:
    if not _check_keys(item, _SCAN_ENTRY_KEYS, label, errors):
        return
    _check_nonnegative_int(item["bytes"], f"{label}.bytes", errors)
    _check_nonnegative_int(item["compressed_bytes"], f"{label}.compressed_bytes", errors)
    if not isinstance(item["is_directory"], bool):
        errors.append(f"{label}.is_directory must be boolean")
    if not isinstance(item["path"], str) or not item["path"]:
        errors.append(f"{label}.path must be a non-empty string")
    if not isinstance(item["crc32"], str):
        errors.append(f"{label}.crc32 must be a string")


def _validate_input_item(item: Any, index: int, errors: list[str]) -> None:
    ...
    for entry_index, entry in enumerate(item["entries"]):
        _validate_scan_entry(entry, f"{label}.entries[{entry_index}]", errors)


def _validate_inputs(manifest: dict[str, Any], errors: list[str]) -> list[dict[str, Any]]:
    inputs = manifest["inputs"]
    if not isinstance(inputs, list):
        errors.append("inputs must be an array")
        return []
    for index, item in enumerate(inputs):
        _validate_input_item(item, index, errors)
    return inputs


def _validate_output_file(item: Any, index: int, errors: list[str]) -> None:
    label = f"outputs.files[{index}]"
    if not _check_keys(item, _FILE_KEYS, label, errors):
        return
    path = Path(item["path"]) if isinstance(item["path"], str) else Path("<invalid>")
    if (
...
def _validate_outputs(
    ...
    if not _check_keys(outputs, _OUTPUT_KEYS, "outputs", errors):
        return {"directory": None, "files": [], "count": 0, "committed": False}, []
    ...
    if not isinstance(files, list):
        ...
        for index, item in enumerate(files):
            _validate_output_file(item, index, errors)
    if (
        not isinstance(outputs["count"], int)
        or isinstance(outputs["count"], bool)
        or outputs["count"] != len(safe_files)
    ):
    ...
    return outputs, safe_files


def _validate_entry(item: Any, index: int, errors: list[str]) -> None:
    ...
    if "source_input_index" in item:
        _check_nonnegative_int(item["source_input_index"], f"{label}.source_input_index", errors)
...
def _validate_entries(manifest: dict[str, Any], errors: list[str]) -> list[dict[str, Any]]:
    entries = manifest["entries"]
    if not isinstance(entries, list):
        errors.append("entries must be an array")
        return []
    for index, item in enumerate(entries):
        _validate_entry(item, index, errors)
    return entries


def _validate_failures_and_claims(manifest: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(failures, list):
        ...
        for index, item in enumerate(failures):
            ...
            if _check_keys(item, _FAILURE_KEYS, label, errors) and not all(
                isinstance(item[key], str) and item[key] for key in _FAILURE_KEYS
            ):
    ...
    if not isinstance(claims, list):
        ...
        for index, item in enumerate(claims):
            ...
            if _check_keys(item, _CLAIM_KEYS, label, errors) and (
                not isinstance(item["claim"], str)
                or item["certainty"] not in {"fact", "strong_inference", "unknown"}
            ):
...
def _validate_state_constraints(
    ...
    if manifest["operation"] == "scan":
        ...
        if manifest["status"] == "failed" and outputs.get("committed"):
            errors.append("failed extract manifests cannot commit output")
        if manifest["status"] != "failed" and not outputs.get("committed"):
        ...
        output_paths = {
            item["path"] for item in safe_files if isinstance(item, dict) and "path" in item
        }
    ...
    if output_root is not None and outputs.get("committed"):
...
def validate_manifest(manifest: Any, output_root: str | Path | None = None) -> list[str]:
    """Validate the core run-manifest contract and optional output hashes."""

    errors: list[str] = []
    if not _check_keys(manifest, _TOP_LEVEL_KEYS, "manifest", errors):
        return errors
    _validate_manifest_metadata(manifest, errors)
    inputs = _validate_inputs(manifest, errors)
